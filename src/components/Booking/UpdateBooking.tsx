      ...
      {selectedBooking && (
        <div className="fixed inset-0 bg-black/75 bg-opacity-50 flex justify-center items-center z-10">
          <div className="bg-gray-500 p-5 rounded-lg w-1/2 h-4/5 overflow-y-auto relative mt-40">
            <button className="absolute top-2 right-2 bg-red-500 text-white px-2 py-1 rounded" onClick={() => setSelectedBooking(null)}>X</button>
            <Formik initialValues={selectedBooking} validationSchema={bookingSchema} onSubmit={handleSubmit} enableReinitialize={true} >
              {({ values, handleBlur, handleChange, errors, touched, handleSubmit }) => (
                <form className="w-4/5" onSubmit={handleSubmit}>
                  ...
                  <div className="mb-3">
                    <input onBlur={handleBlur} onChange={handleChange} value={values.pickup_location ?? ''} name="pickup_location" type="text" placeholder="Enter Pickup Location" className={`rounded-lg border-2 focus:outline-none focus:border-blue-700 p-2 w-full mb-3 ${errors.pickup_location && touched.pickup_location && 'border-red-700'}`} />
                    {errors.pickup_location && touched.pickup_location && errors.pickup_location ? <div className="text-red-600">{errors.pickup_location}</div> : null}
                  </div>
                  <div className="mb-3">
                    <input onBlur={handleBlur} onChange={handleChange} value={values.dropoff_location ?? ''} name="dropoff_location" type="text" placeholder="Enter Dropoff Location" className={`rounded-lg border-2 focus:outline-none focus:border-blue-700 p-2 w-full mb-3 ${errors.dropoff_location && touched.dropoff_location && 'border-red-700'}`} />
                    {errors.dropoff_location && touched.dropoff_location && errors.dropoff_location ? <div className="text-red-600">{errors.dropoff_location}</div> : null}
                  </div>
                  <div className="mb-3">
                  ...
                  <div className="text-center mt-3 ">
                    <button type="submit" className="bg-green-500 rounded-xl px-5 py-2">Update Booking</button>
                  </div>
                </form>
              )}
            </Formik>
